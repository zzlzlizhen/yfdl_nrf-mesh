...
import com.phyplusinc.android.phymeshprovisioner.ble.BleMeshManagerCallbacks;
...
import static no.nordicsemi.android.meshprovisioner.MeshManagerApi.MESH_PROVISIONING_UUID;
import static com.phyplusinc.android.phymeshprovisioner.ble.BleMeshManager.MESH_PROXY_UUID;
...
public class NrfMeshRepository implements MeshProvisioningStatusCallbacks, MeshStatusCallbacks, MeshManagerCallbacks, BleMeshManagerCallbacks {
    ...
    private static final int MSDELAY_PROV_CFGS = 500;
    public static final String EXPORT_PATH = Environment.getExternalStorageDirectory() + File.separator +
            "PhyPlusInc" + File.separator + "PhyMesh" + File.separator;
    private static final String EXPORTED_PATH = "sdcard" + File.separator + "PhyPlusInc" + File.separator + "PhyMesh" + File.separator;
    ...
    // >>>, PANDA, ADD,
    /**
     * returns the provisioned Mesh Node
     */
    LiveData<ProvisionedMeshNode> getProvMeshNode() {
    ...
    public void onDeviceDisconnected(@NonNull final BluetoothDevice device) {
        Log.i(TAG, "onDeviceDisconnected:" + "Device Disconnected !!!");

        // >>>, PANDA, re-conn on error
        //if (mFlagConnRtry) {
        //    mFlagConnRtry = false;
        //    mBleMeshManager.setConn(device);
        //    return;
        //}
        // <<<,
    ...
    //private boolean mFlagConnRtry = false;
    @Override
    public void onError(final BluetoothDevice device, @NonNull final String message, final int errorCode) {
        ...
        // >>>, ADD, PANDA, disconnect on selected error.
        // TODO: filter some error for auto disconnecting: MTU:133
        //mFlagConnRtry = true;
        disconnect();
    }
    ...
    private void onProvisioningCompleted(final ProvisionedMeshNode node) {
        ...
        mHandler.postDelayed(mReconnectRunnable, /*1000*/2000); //Added a slight delay to disconnect and refresh the cache
    }
            ...
            if (meshMessage instanceof ConfigCompositionDataStatus) {
                ...
                if (mSetupProvisionedNode) {
                    if (status.isSuccessful()) {
                        // >>>, ADD, PANDA, We auto bind app key to all model for Phy+
                        final Integer cid = mProvisionedMeshNode.getCompanyIdentifier();
                        final Integer pid = mProvisionedMeshNode.getProductIdentifier();

                        if ( 0x0504 == cid && 0x6212 == pid ) {
                            // TODO, PANDA, revise??
                            mProvisionedMeshNode.setAppKeyBindPhy(status.getAppKeyIndex());
                            mSetupProvisionedNode = false;
                            mIsNetworkRetransmitSetCompleted = true;

                            Log.d(TAG, "onMeshMessageReceived: " + "ConfigAppKeyStatus: Success! " + "Provisioning Completed Now for PHY+");
                        } else {
                            // <<<
    ...
    private final ScanCallback scanCallback = new ScanCallback() {
        ...
        public void onScanResult(final int callbackType, final ScanResult result) {
            ...
            // >>>, FIX, PANDA, node unmatched with provisioning node
            final String deviAddr = result.getDevice().getAddress();
            if ( deviAddr.equals(mDeviAddr) && scanRecord != null ) {
                // <<<,
    ...
    private void onProvisionedDeviceFound(final ProvisionedMeshNode node, final ExtendedBluetoothDevice device) {
        ...
        mHandler.postDelayed(() -> connectToProxy(device), /*2000*/1000);
    }
